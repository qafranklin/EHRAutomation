...
test('@authregression Cancel New Message', async ({ page }) => {
  ...
  ismsgbody.fill('Good morning Mam Sir');
   ...
   const iscancelbtn = page.locator('#btnCancel')
   await expect(iscancelbtn).toBeVisible();
   await expect(iscancelbtn).toBeEnabled()
   iscancelbtn.click();

   //Verify New Message Modal 
   await expect(page.locator('#new-message-modal')).toBeHidden();
