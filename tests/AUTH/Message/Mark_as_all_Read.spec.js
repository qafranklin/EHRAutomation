...
test('@authregression Mark as all Unread', async ({ page }) => {
  ...
  const ismarkmsguread =  page.locator('#btnMarkAsRead')
  await expect(ismarkmsguread).toBeVisible();
  await expect(ismarkmsguread).toBeEnabled();
  ismarkmsguread.click();
  console.log('Mark as all Read Clicked')
  ...
  await expect(ismsgcontentrowfirst).not.toHaveClass('unread-conversation')
